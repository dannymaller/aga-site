...
/** No filters means everyone. Otherwise a member counts if they match any. */
function matches(m) {
  if (!activeFilters.size) return true;
  for (const f of activeFilters) if (matchesOne(m, f)) return true;
  return false;
}
...
function render() {
  ...
  $("#count").textContent = members.length
    ? n + (n === 1 ? " member" : " members") + (activeFilters.size ? " match" : " on the map")
    : "";
  ...
  chip.addEventListener("click", () => {
    ...
    $$(".chip").forEach(c => {
      const on = c.dataset.filter === "all"
        ...
        : activeFilters.has(c.dataset.filter);
